...
function theEliminator(contestants, loser) {
  for(let i = 0; i < contestants.length; i++) {
    (contestants[i] === loser? contestants.splice(i, 1) : '')
    }
  return contestants
}
...
function emailCheck(email) {
  let emailRaw = String(email)
  let emailRefined = emailRaw.trim()
   return emailRefined.includes(`@`)? ('email verified'): ('must provide a vaild email address')
  }
 /* let email = 'smokeyb3ar@aol.com'
...
function potterWorldScam(gold) {
  let totalChocoFrogs = gold / 3
  return totalChocoFrogs
}

let totalGold = 44
let totalFrogs = potterWorldScam(totalGold)
